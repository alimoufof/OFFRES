...
@section('header','Ajout d\'un Nouveau Domaine')
                        ...
                        <form action="{{ route('domaine.store') }}" method="post">
                            ...
                            <div class="row">
                                <div class="col-xl-6  col-md-6 mb-4">
                                    ...
                                    <select class="form-control @error('departement_id') is-invalid @enderror solid"
                                        name="departement_id" id="departement_id">
                                        <option value="">Veuillez choisir un département</option>
                                        @foreach ($departements as $key => $departement)
                                            <option value="{{ $key }}"
                                                {{ old('departement_id') == $key ? 'selected' : '' }}>
                                                {{ $departement }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-xl-6  col-md-6 mb-4">
                                    <label for="nom_domaine" class="form-label">domaine <span
                                            class="text-danger scale5 ms-2">*</span></label>
                                    <input type="text"
                                        class="form-control @error('nom_domaine') is-invalid @enderror solid"
                                        name="nom_domaine" id="nom_domaine" placeholder="Nom domaine" aria-label="name"
